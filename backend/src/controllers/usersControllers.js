...
exports.createUser = (req, res) => {
	...
	connection.query(
		`INSERT INTO users (username, full_name, role, password, gmail, date) VALUES (?, ?, 1, ?, ?, CURRENT_TIMESTAMP)`,
		[username, full_name, password, gmail],
		(error, result) => {
			if (error) {
...
exports.login = (req, res) => {
	...
	connection.query(
		...
		(error, results) => {
			...
			res.json(
				results[0].password === reqPassword
					? "Login Success"
					: "Login failed"
			);
		}
